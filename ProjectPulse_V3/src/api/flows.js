/* ============================================================
    ...
    the exact keys its own flow branch produces):
      ...
      Invoice         -> "guid","invoiceNumber","clientId","projectId","amount","dueDate","invoiceDate","currencyId","billingPeriodId","totalBillableHours","invoiceStatusId","paymentDate","paymentReference","financeRemarks","billingId","active"   (guid = "InvoiceId"; flow1/FLOW_URL, not flow2)
    ============================================================ */
...
export function callAuditEmailFlow(entry) {
  ...
  /* ============================================================
    INVOICE FLOW — entity="Invoice". Lives on flow1/FLOW_URL
    alongside Project/Client/InvoiceStatus (the Invoice table is
    one of the original 17 live tables, not a gap-closure one).
    Required on the SQL side: InvoiceNumber, ClientId, Amount.
    Everything else (ProjectId, DueDate, InvoiceDate, CurrencyId,
    BillingPeriodId, TotalBillableHours, InvoiceStatusId,
    PaymentDate, PaymentReference, FinanceRemarks, BillingId) is
    nullable per the live schema, so it's sent as-is / null.
    ============================================================ */
  export function callInvoiceFlow(action, item = {}) {
    ...
    return postFlow(body, "Invoice").then((list) => {
      const normalized = list.map((d, i) => {
        const guid = normGuid(d.guid);
        return {
          id: guid !== "" ? guid : String(i),
  ...
  const REFERENCE_RULES = {
    ...
    User: [
      ...
      { refEntity: "UserRoles", field: "userId", label: "user role assignments" },
