...
import { callDepartmentFlow, callUserFlow, callLocationFlow, callDesignationFlow, checkReferences } from "../../api/flows";
...
const EMPTY_FORM = { guid: "", empId: "", firstName: "", lastName: "", gender: "", designationId: "", departmentId: "", email: "", reportingManagerId: "", locationId: "", active: true };
...
export function UsersPage() {
  ...
  const designationName = (id) => designations.find((d) => String(d.id) === String(id))?.name || "—";
  ...
  const filtered = rows
    .filter((r) => {
      ...
      return (r.empId || "").toLowerCase().includes(q)
        ...
        || designationName(r.designationId).toLowerCase().includes(q);
    })
  ...
  const openEdit = async (u) => {
    ...
    setPanel({ mode: "edit", data: { ...u }, restricted: guard.blocked });
  };

  const confirmDeleteRow = async () => {
    if (!confirmDelete) return;
    const guard = await checkReferences("User", confirmDelete.guid);
    if (guard.blocked) {
    ...
    callUserFlow("DELETE", confirmDelete)
      .then((res) => {
        ...
        setDeleting(false);
        setConfirmDelete(null);
        logAudit("User", "Delete", `${confirmDelete.firstName || ""} ${confirmDelete.lastName || ""}`.trim() || confirmDelete.empId || "record");
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ background: COLORS.bg }}>
                {["Emp ID", "Name", "Designation", "Department", "Gender", "Status", "Actions"].map((h) => (
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={u.id} style={{ borderTop: `1px solid ${COLORS.border}`, background: i % 2 ? "#FAFBFD" : "#fff" }}>
                  ...
                  <td style={{ padding: "11px 16px", fontSize: 13.5, color: COLORS.text }}>{designationName(u.designationId)}</td>
